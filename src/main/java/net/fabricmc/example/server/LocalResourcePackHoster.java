...
public class LocalResourcePackHoster extends Thread {
    ...
    public Pair<InputStream,Long> requestFileCallback(MineConnection connection, String request) {
        ...
        for (var p : EmbeddedAssetsServer.server.getPlayerManager().getPlayerList())
            if (p.getIp().equals(clientAddrs) || p.getIp().equals("127.0.0.1"))
    ...
    public class MineConnection implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (get.matches()) {
                    request = get.group(1);
                    Pair<InputStream,Long> result = requestFileCallback(this, request);
                    if (result == null) {
